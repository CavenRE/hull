...
(function () {
  ...
  function action(act, name) {
    ...
    if (act === "start")     { i.status = "running"; App.toast(`Started ${name}`); renderServices(rootEl); }
    else if (act === "stop") { i.status = "stopped"; App.toast(`Stopped ${name}`); renderServices(rootEl); }
  ...
  function openAddInstance() {
            ...
            <div class="cat-group">
              ...
              ${g.items.map(it => {
                ...
                return `<div class="engine-opt" data-engine="${it.engine}">
                  ...
                  <select class="select eo-ver" style="width:auto" onclick="event.stopPropagation()">${it.versions.map(v=>`<option>${v}</option>`).join("")}</select>
                </div>`;
              }).join("")}
            </div>`).join("")}
        </div>
    ...
    create.addEventListener("click", () => {
      App.closeDialog();
      App.toast(`Creating ${H().ENGINES[sel].label} instance…`);
    });
  }
  ...
  function openEdit(i) {
      ...
      <div class="dialog dialog-lg">
        ...
        <div class="dialog-body">
          <div class="form-grid form-row">
            ...
            <div><label class="field-label">Version</label><select class="select">${versions.map(v=>`<option ${v===i.version?"selected":""}>${v}</option>`).join("")}</select></div>
          </div>
          <div class="form-grid form-row">
    ...
    document.getElementById("saveInst").addEventListener("click", () => { App.closeDialog(); App.toast(`Saved ${i.name}`); });
  }
  ...
  function openOpenWith(i) {
    ...
    document.querySelectorAll("[data-tool]").forEach(o =>
      o.addEventListener("click", () => { App.closeDialog(); App.toast(`Opening ${i.name} in ${o.dataset.tool}`); }));
  }
  ...
  function openLinkProject(i) {
    ...
    document.querySelectorAll("[data-site]").forEach(o =>
      o.addEventListener("click", () => { App.closeDialog(); App.toast(`Linked ${i.name} to ${o.dataset.site}`); }));
  }
  ...
  function openDestroy(i) {
    ...
    db.addEventListener("click", () => { App.closeDialog(); App.toast(`Destroyed ${i.name}`); });
  }
})();
