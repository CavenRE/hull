...
Password   (none)`;
  ...
  function renderNoService(body) {
    ...
    body.querySelector('[data-act="add"]').addEventListener("click", () => {
      H().SERVICES.push({ name: "mailpit", engine: "mailpit", version: "latest", status: "running",
        host: "127.0.0.1", host_port: 1025, username: null, password: "", url: "http://localhost:8025", linked: [] });
      App.toast("Mailpit added · catching mail");
      window.renderMail(rootEl);
    });
  }
  ...
  function renderSurface(body, m) {
    ...
    if (openBtn) openBtn.addEventListener("click", () => App.toast("Opening Mailpit inbox in a new window…"));

    body.querySelectorAll("[data-act]").forEach(b => b.addEventListener("click", () => {
      const a = b.dataset.act;
      if (a === "start") { m.status = "running"; App.toast("Mailpit started"); }
      if (a === "stop")  { m.status = "stopped"; App.toast("Mailpit stopped"); }
      window.renderMail(rootEl);
    }));
    ...
    body.querySelectorAll("[data-wire]").forEach(b => b.addEventListener("click", () => {
      m.linked.push(b.dataset.wire); App.toast(`Wired ${b.dataset.wire} to Mailpit`); window.renderMail(rootEl);
    }));
    body.querySelectorAll("[data-unwire]").forEach(b => b.addEventListener("click", () => {
      m.linked = m.linked.filter(n => n !== b.dataset.unwire); App.toast(`Unwired ${b.dataset.unwire}`); window.renderMail(rootEl);
    }));
  }
})();
