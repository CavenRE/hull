...
(function () {
  ...
  function foldersCard() {
    return `
      ...
      <div class="card" style="margin-bottom:24px">
        ...
        ${H().DIRS.map(d => `<div class="folder-row">
        ...
        <div style="margin-top:12px"><button class="btn btn-sm" id="addFolder">${icon("plus",14)}Add folder…</button></div>
      </div>`;
  }

  function defaultsCard() {
    const sel = (label, opts) => `<div><label class="field-label">${label}</label><select class="select">${opts.map(o=>`<option>${o}</option>`).join("")}</select></div>`;
    return `
      <div class="section-label">Defaults</div>
      <div class="card" style="margin-bottom:24px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          ${sel("Default PHP version", ["8.3","8.2","8.1","8.0"])}
          ${sel("Open in editor", ["VS Code","PhpStorm","Cursor","Sublime Text","Zed","Custom…"])}
          ${sel("Database tool", ["TablePlus","DBeaver","Adminer (web)","CLI"])}
          ${sel("Default web server", ["nginx","Apache","Caddy"])}
        </div>
      </div>`;
  }
  ...
  function doctorCard() {
    return `
      ...
      <div class="card" style="margin-bottom:24px">
        ...
        ${DOCTOR.map(c => `<div class="doctor-row">
          <span style="color:${DXCOLOR[c.status]};margin-top:1px">${icon(DXICON[c.status],16)}</span>
  ...
  function wire(el) {
    ...
    el.querySelector("#addFolder")?.addEventListener("click", t("Choose a folder to scan…"));
    el.querySelectorAll("[data-folder-remove]").forEach(b => b.addEventListener("click", () => App.toast(`Removed ${b.dataset.folderRemove} folder`)));
    el.querySelector("#rerunSetup")?.addEventListener("click", t("Re-running setup…"));
    ...
    el.querySelectorAll("[data-install]").forEach(b => b.addEventListener("click", () => {
      const d = H().DEPENDENCIES.find(x => x.key === b.dataset.install); d.installed = d.latest; d.status = "ok";
      App.toast(`Installing ${d.name}…`); window.renderSettings(rootEl);
    }));
    el.querySelectorAll("[data-startup]").forEach(b => b.addEventListener("change", () => App.toast(b.checked ? "Enabled" : "Disabled")));
    el.querySelector("#runDoctor")?.addEventListener("click", t("Running checks…"));
